...
func BuildForest(rs rand.Source, inputs [][]float64,labels []float64, treesAmount, samplesAmount, selectedFeatureAmount int) *Forest{
	forest := &Forest{}
	...
	for i:=0;i<treesAmount;i++{
		go func(x int){
			...
			forest.Trees[x] = BuildTree(rs, inputs,labels,samplesAmount,selectedFeatureAmount)
...
func DefaultForest(rs rand.Source, inputs [][]float64,labels []float64, treesAmount int) *Forest{
	m := int( math.Sqrt( float64( len(inputs[0]) ) ) )
	n := int( math.Sqrt( float64( len(inputs) ) )  )
	return BuildForest(rs, inputs,labels, treesAmount,n,m)
}

func (self *Forest) Predicate(input []float64) float64{
	total := 0.0
